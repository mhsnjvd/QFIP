...
The hazard rate (also called default intensity) is the probability 
of default for a certain time period \emph{conditional} on no earlier default.
In general, the hazard rate will be different when different time periods
are considered. Note that hazard rate is a probability. 

Let $X$ be the random variable representing the time (in years)
when the company defaults. For a simple hazard rate model, 
we assume that 
$$P( X \leq t ) = 1 - \exp( -h t ),$$ i.e.\
$X$ is exponentially distributed with $h$ can be though 
of as the average hazard rate.
The interpretation of $h$ is given by the equation,
$$P( t \leq X \leq t + \delta t | X > t ) \approx h \delta t. $$
In other words, for this model, the hazard rate (i.e.\ probability of default) 
for a small period $\delta t$ is $h \delta t$
