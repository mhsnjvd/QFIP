...
Risk neutral probability of a financial
contract should be called \emph{market price probability}, 
i.e.\ the probability inferred from the price 
that the market is willing to pay for the contract.

Let $B$ be a binary contract, which 
pays \$1 at time $T$ if an event $E$ occurs and 
nothing if $E$ doesn't occur. Then the 
...
Let us assume that the price of a stock 
follows geometric Brownian motion, and the 
price of the stock at time $t$ is $S(t)$.

The stochastic differential equation followed
...
\begin{equation*}
S(u) = S(t)\exp\left((\mu - \frac{1}{2}\sigma^2)(u-t)+\sigma\sqrt{(u-t)}Z\right).
\end{equation*}
...
$S$ at time $t$ is greater than or equal to the strike price $K$? $N(d_2)$.
