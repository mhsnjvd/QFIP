...
\begin{align*}
\mathrm{E}_{XY}[(Y-r(X))^2] &= \mathrm{E}_X[\mathrm{E}_{Y|X}[(Y-r(X))^2]]\\
&\geq \mathrm{E}_X[\mathrm{E}_{Y|X}[(Y-r_*(X))^2]]\\
...
Since projection error is orthogonal to the column 
space of $A$, the least square problem boils down to,
\begin{align*}
...
\end{align*}
Since $A$ has full column rank, its null space is trivial and 
therefore,
\begin{align*}
x = (A^*A)^{-1}A^*b.
\end{align*}
...
is $QQ^*$. The QR decomposition of $A$ together with the projection matrix 
$QQ^*$ lead to another algorithm for least squares problem. We have,
\begin{align*}
Ax &= Proj_{Col(A)}(b)\\
QRx &= QQ^*b\\
Rx &= Q^*b\\
