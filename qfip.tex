...
are given by a $n$-vector 
$r =[a_1, a_2, \ldots, a_n]^T$.
...
$w_i$ is the characteristic portfolio of the attribute vector $a_i$. 
...
in $\mathbb{R}^n$ and $\Sigma$ is 
an $\mathbb{R}^{n\times n}$ 
symmetric positive definite covariance
matrix obtained from a factor model and 
$\lambda \geq 0$ is the risk aversion 
parameter. 

Our aim is to solve a constrained mean-variance optimization
...
\begin{align*}
r_{\pi} &= \underbrace{\sqrt{n-1}}_\text{Breadth}
\underbrace{\mathrm{Corr}(\hat{\alpha}, \hat{\epsilon})}_\text{Skill}
\underbrace{\sigma_{r_{\pi}}}_\text{Traget risk} 
...
Linear regression assumes a linear model, i.e.,
$X$ and $Y$ are linearly related:

\begin{align*}
Y = \beta_0 + \beta_1 X + \epsilon,
\end{align*}
where $\epsilon$ is a noise term, and 
furthermore, linear regression assumes that the regression function 
or the conditional expectation is \emph{exactly} linear in $X$, i.e.\
the conditional expectation of the noise term is zero:
\begin{align*}
...
\begin{enumerate}
\item residual $\epsilon$ has zero conditional mean: $\mathrm{E}[\epsilon|X] = 0$
\item residuals are uncorrelated: 
$\mathrm{E}[\epsilon_i \epsilon_j |X] = 0$ for $i \neq j$.  
\item residuals have a constant variance: $\mathrm{E}[\epsilon^2|X] = 
