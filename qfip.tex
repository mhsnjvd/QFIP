...
number of columns of $A$. Then columns of 
$A$ are a basis of $\mathbb{R}^{n}$ since,
\begin{enumerate}
\item Each column of $A$ is in $\mathbb{R}^n$ since $A$ is square. (This is the only place where we use the fact that $A$ is square).
\item Columns of $A$ form a set of $n$ linearly independent vectors in $\mathbb{R}^n$.
\item Columns of $A$ are a maximally independent set in $\mathbb{R}^n$ and therefore must span $\mathbb{R}^n$.
\end{enumerate}
...
The solution is unique, for if $x'$ is another solution, then,
\begin{align*}
Ax' &= y = Ax\\
x'&= B_Ly = x.
\end{align*}
...
x = (A^*A)^{-1}A^*b.
\end{align*}
Equivalently, we can re-write the above as 
\begin{align*}
