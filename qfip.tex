...
r &= \mathrm{E}[Y].
\end{align*}
Note also that with $r=\mathrm{E}[Y]$, the MSE is $\mathrm{E}[(Y-\mathrm{E}[Y])^2]=\mathrm{Var}[Y]$.
...
\begin{align*}
\mathrm{E}_{XY}[(Y-c(X))^2] &= \mathrm{E}_X[\mathrm{E}_{Y|X}[(Y-c(X))^2]]\\
&\leq \mathrm{E}_X[\mathrm{E}_{Y|X}[(Y-\mathrm{E}[Y|X])^2]]\\
